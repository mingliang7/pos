...
itemsTmpl.helpers({
    ...
    tableSettings: function () {
        let i18nPrefix = 'pos.companyExchangeRingPull.schema';
...
itemsTmpl.events({
        ...
        if (invoice) {
            ...
            Meteor.call('findStockRingPull', itemId, branchId, function (error, qtyResult) {
                ...
                if (itemOfCollectionNull) {
                    checkQty = math.round(qty + parseFloat(itemOfCollectionNull.qty), 3);
                } else {
                    checkQty = qty;
                }
                qtyResult += soldQty;
    ...
    'click .js-update-item': function (event, instance) {
        alertify.item(fa('pencil', TAPi18n.__('pos.companyExchangeRingPull.schema.itemId.label')), renderTemplate(editItemsTmpl, this));
    },
        ...
        if (AutoForm.getFormId() == "Pos_companyExchangeRingPullUpdate") { //check if update form
            ...
            destroyAction(
                itemsCollection, {
                    ...
                    title: TAPi18n.__('pos.companyExchangeRingPull.schema.itemId.label'),
...
AutoForm.addHooks(['Pos_companyExchangeRingPullItemsEdit'], hooksObject);
