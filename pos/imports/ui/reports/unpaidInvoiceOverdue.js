...
//page
import './unpaidInoviceOverdue.html';
//import DI
import  'printthis';
//import collection
import {unpaidInvoiceOverdueSchema} from '../../api/collections/reports/unpaidInvoiceOverdue';

//methods
import {unpaidInvoiceOverdue} from '../../../common/methods/reports/unpaidInvoiceOverdue';
//state
let paramsState = new ReactiveVar();
let invoiceData = new ReactiveVar();
//declare template
let indexTmpl = Template.Pos_unpaidInvoiceOverdue,
    invoiceDataTmpl = Template.unpaidInvoiceOverdueData;
Tracker.autorun(function () {
    if (paramsState.get()) {
        ...
        unpaidInvoiceOverdue.callPromise(paramsState.get())
...
indexTmpl.helpers({
    schema(){
        return unpaidInvoiceOverdueSchema;
    }
});
indexTmpl.events({
...
invoiceDataTmpl.helpers({
    ...
    getTotal(dueAmount, paidAmount, balanceAmount, customerName){
        ...
        string += `
                ...
                <td><u>${numeral(dueAmount).format('0,0.0000')}</u></td>
                <td><u>${numeral(paidAmount).format('0,0.0000')}</u></td>
                <td><u>${numeral(balanceAmount).format('0,0.0000')}</u></td>
                `;
        return string;
    },
