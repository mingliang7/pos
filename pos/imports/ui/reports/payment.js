...
receivePaymentTmpl.helpers({
    ...
    getTotal(dueAmount, paidAmount, balanceAmount){
        ...
        string += `<td><b>Total:</td></b><td><b>${numeral(dueAmount).format('0,0.0000')}</b></td>`;
        string += `<td><b>${numeral(paidAmount).format('0,0.0000')}</b></td>`;
        string += `<td><b>${numeral(balanceAmount).format('0,0.0000')}</b></td>`;
        return string;
    }
});
